...
describe("metadata microservice it", () => {
    ...
    // 
    // Helper function to load a database collection into our database.
    //
    async function loadDatabaseCollection(collectionName, records) {
        await microservice.db.dropDatabase(); // Reset the test database.

        const collection = microservice.db.collection(collectionName);
        await collection.insertMany(records); // Insert the database collection.
    }
    ...
    test("/videos route retrieves data via videos collection", async () => {
        ...
        // Load database collection into the database.
        await loadDatabaseCollection("videos", testVideos);
